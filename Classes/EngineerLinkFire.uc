...
/*                          if (!HealObjective.HealDamage(AdjustedDamage, Instigator.Controller, DamageType))
        ...
        if(LockedPawn!=None) //giving shields or health to someone we are linking to
        {
            ...
            EGun = WeaponModifier_EngineerLink(class'WeaponModifier_EngineerLink'.static.GetFor(Weapon));
            if(EGun!=None)
                EGun.AdjustTargetDamage(AdjustedDamage,AdjustedDamage,LockedPawn,Instigator,LockedPawn.Location,Momentum,class'DamTypeLinkShaft');
        }
